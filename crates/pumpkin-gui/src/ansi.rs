...
#[cfg(test)]
mod tests {
    use super::{DEFAULT_LINK_COLOR, render};

    #[test]
    fn plain_text_passes_through_unstyled() {
        let out = render("hello world");
        assert_eq!(out.plain, "hello world");
        assert_eq!(out.html, "hello world");
    }

    #[test]
    fn strips_named_colour_and_bold() {
        let out = render("\x1b[1m\x1b[32mGreen bold\x1b[0m plain");
        assert_eq!(out.plain, "Green bold plain");
        assert_eq!(
            out.html,
            "<span style=\"color:#00cd00;font-weight:bold;\">Green bold</span> plain"
        );
    }

    #[test]
    fn strips_truecolour() {
        let out = render("\x1b[38;2;10;20;30mcustom\x1b[0m");
        assert_eq!(out.plain, "custom");
        assert_eq!(out.html, "<span style=\"color:#0a141e;\">custom</span>");
    }

    #[test]
    fn osc8_hyperlink_becomes_anchor() {
        let out = render("\x1b]8;;https://pumpkinmc.org\x1b\\click here\x1b]8;;\x1b\\ done");
        assert_eq!(out.plain, "click here done");
        assert_eq!(
            out.html,
            format!(
                "<a href=\"https://pumpkinmc.org\" style=\"text-decoration:underline;color:{DEFAULT_LINK_COLOR};\">click here</a> done"
            )
        );
    }

    #[test]
    fn bare_url_is_auto_linkified() {
        let out = render("see https://example.com/path for details.");
        assert_eq!(out.plain, "see https://example.com/path for details.");
        assert_eq!(
            out.html,
            format!(
                "see <a href=\"https://example.com/path\" style=\"text-decoration:underline;color:{DEFAULT_LINK_COLOR};\">https://example.com/path</a> for details."
            )
        );
    }
    ...
    fn html_special_characters_are_escaped() {
        let out = render("<script>&\"'</script>");
        assert_eq!(out.plain, "<script>&\"'</script>");
        assert_eq!(out.html, "&lt;script&gt;&amp;&quot;'&lt;/script&gt;");
    }

    #[test]
    fn malicious_url_cannot_break_out_of_the_href_attribute() {
        let out = render("https://evil.example/\"onclick=\"alert(1)");
        assert!(!out.html.contains("\"onclick="));
    }

    #[test]
    fn colour_and_link_combine() {
        let out = render("\x1b[31m\x1b]8;;https://x.test\x1b\\red link\x1b]8;;\x1b\\\x1b[0m");
        assert_eq!(out.plain, "red link");
        assert_eq!(
            out.html,
            "<span style=\"color:#cd0000;\"><a href=\"https://x.test\" style=\"text-decoration:underline;color:#cd0000;\">red link</a></span>"
        );
    }
}
