...
impl TurtleEggBlock {
    ...
    pub async fn destroy_egg(
        ...
        {
            Self::decrease_eggs(world, pos, state_id, block).await;
        }
    }
}
        ...
        Box::pin(async move {
            ...
            if args.entity.get_entity().entity_type.id != EntityType::ZOMBIE.id {
                ...
                if block == &Block::TURTLE_EGG {
                    Self::destroy_egg(args.world, &pos, state.id, block, args.entity, 3).await;
                }
            }
        })
    }
