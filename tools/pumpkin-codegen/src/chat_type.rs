...
struct ChatTypeJson {
    chat: ChatFormatJson,
    narration: Option<ChatFormatJson>,
}

#[derive(Deserialize)]
struct ChatFormatJson {
...
pub fn build() -> TokenStream {
    ...
    for entry in entries {
        ...
        let content = fs::read_to_string(&path).expect("read chat_type file");
        let json: ChatTypeJson = serde_json::from_str(&content).expect("parse chat_type JSON");

        chat_types.insert(stem, json);
    }
