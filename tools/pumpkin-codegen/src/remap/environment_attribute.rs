...
/// Generates the `TokenStream` for per-version environment attribute ID remap tables and the
/// `remap_environment_attribute_id_for_version` function.
pub fn build() -> TokenStream {
    let remapper: Remapper<_, Option<Vec<u32>>> = Remapper {
        ...
        serializer: |&file| {
            ParsedMappings::parse_mapping_file(file, "environment_attribute")
    ...
    for (ver, mapping) in &all_mappings {
        ...
        let ident = format_ident!(
            "{}",
            format!(
                "ENVIRONMENT_ATTRIBUTE_ID_REMAP_{:?}_TO_{:?}",
