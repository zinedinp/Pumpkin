...
/// Generates the `TokenStream` for per-version painting variant ID remap tables and the
/// `remap_painting_variant_id_for_version`/`remap_motive_id_for_version` functions.
pub fn build() -> TokenStream {
    let remapper: Remapper<_, Option<Vec<u32>>> = Remapper {
        ...
        serializer: |&file| {
            ParsedMappings::parse_mapping_file(file, "paintings")
    ...
    for (ver, mapping) in &all_mappings {
        ...
        let ident = format_ident!(
            "{}",
            format!(
                "PAINTING_VARIANT_ID_REMAP_{:?}_TO_{:?}",
    ...
    quote! {
        ...
        #[must_use]
        pub fn remap_motive_id_for_version(
            motive_id: u32,
            version: JavaMinecraftVersion,
        ) -> u32 {
