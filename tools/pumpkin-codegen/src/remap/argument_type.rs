...
/// Generates the `TokenStream` for per-version command argument type ID remap tables and the
/// `remap_argument_type_id_for_version` function.
pub fn build() -> TokenStream {
    ...
    let remapper: Remapper<_, Option<Vec<u32>>> = Remapper {
        ...
        remapper: |first, second| match (first, second) {
            (Some(first), Some(second)) => Some(
                first
                    .iter()
                    .map(|&id| second.get(id as usize).copied().unwrap_or(5))
                    .collect(),
            ),
        ...
        serializer: |&file| {
            ParsedMappings::parse_mapping_file(file, "argumenttypes").map(|mappings| {
                mappings
                    .forward
                    .iter()
                    .map(|&x| if x < 0 { 5 } else { x as u32 })
                    .collect()
            })
        },
    };
    ...
    let default_mapping: Vec<u32> = (0..60).collect();

    for (ver, mapping) in &all_mappings {
        let ident = format_ident!(
            "{}",
            format!("ARGUMENT_TYPE_ID_REMAP_{:?}_TO_{:?}", remapper.version, ver).to_uppercase()
        );
        let mapping_vec = mapping.as_ref().unwrap_or(&default_mapping);
        let mapping_tokens: Vec<_> = mapping_vec
            .iter()
            .copied()
            .map(Literal::u32_unsuffixed)
            .collect();
        static_values.extend(quote! {
            const #ident: &[u32] = &[#(#mapping_tokens),*];
        });
