...
/// Generates the `TokenStream` for per-version slot display ID remap tables and the
/// `remap_slot_display_id_for_version` function.
pub fn build() -> TokenStream {
    let remapper: Remapper<_, Option<Vec<u32>>> = Remapper {
        ...
        serializer: |&file| {
            ParsedMappings::parse_mapping_file(file, "slot_displays")
    ...
    for (ver, mapping) in &all_mappings {
        ...
        let ident = format_ident!(
            "{}",
            format!("SLOT_DISPLAY_ID_REMAP_{:?}_TO_{:?}", remapper.version, ver).to_uppercase()
        );
