...
/// Generates the `TokenStream` for per-version recipe serializer ID remap tables and the
/// `remap_recipe_serializer_id_for_version` function.
pub fn build() -> TokenStream {
    ...
    let remapper: Remapper<_, Option<Vec<u16>>> = Remapper {
        ...
        serializer: |&file| {
            ParsedMappings::parse_mapping_file(file, "recipe_serializers")
    ...
    let max_size = all_mappings
        .iter()
        .filter_map(|(_, m)| m.as_ref().map(Vec::len))
        .max()
        .unwrap_or(32);
    let default_mapping: Vec<u16> = (0..max_size as u16).collect();

    for (ver, mapping) in &all_mappings {
        let ident = format_ident!(
            "{}",
            format!(
                "RECIPE_SERIALIZER_ID_REMAP_{:?}_TO_{:?}",
