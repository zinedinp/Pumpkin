...
/// Generates the `TokenStream` for per-version block entity type ID remap tables and the
/// `remap_block_entity_type_id_for_version` function.
pub fn build() -> TokenStream {
    let remapper: Remapper<_, Option<Vec<u32>>> = Remapper {
        ...
        serializer: |&file| {
            ParsedMappings::parse_mapping_file(file, "blockentities")
    ...
    for (ver, mapping) in &all_mappings {
        ...
        let ident = format_ident!(
            "{}",
            format!(
                "BLOCK_ENTITY_TYPE_ID_REMAP_{:?}_TO_{:?}",
