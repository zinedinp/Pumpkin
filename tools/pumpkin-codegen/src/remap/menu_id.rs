...
/// Generates the `TokenStream` for per-version menu/screen ID remap tables and the
/// `remap_menu_id_for_version` function.
pub fn build() -> TokenStream {
    ...
    let remapper: Remapper<_, Option<Vec<u8>>> = Remapper {
        ...
        serializer: |&file| {
            ParsedMappings::parse_mapping_file(file, "menus").map(|mappings| {
                mappings
                    .forward
                    .iter()
    ...
    for (ver, mapping) in &all_mappings {
        let ident = format_ident!(
            "{}",
            format!("MENU_ID_REMAP_{:?}_TO_{:?}", remapper.version, ver).to_uppercase()
        );
    ...
    quote! {
        ...
        #[must_use]
        pub fn remap_menu_id_for_version(
            menu_id: u8,
